...
export const site = {
  ...
  /**
   * Official EMS mark, derived from brand/EMS_Icon_Reversed_On_Ink.pdf.
   *
   * `logo` is the mark on a transparent background, so it sits on any dark
   * surface without a visible panel behind it. `icon` keeps the official
   * "reversed on ink" presentation, which is what a favicon or app tile wants.
   * Both are square (1:1) — size them equally in both dimensions.
   */
  logo: {
    src: '/ems-logo.png',
  ...
  /**
   * Colours sampled from the official artwork. Recorded here as the canonical
   * brand values; the site's own palette lives in app/globals.css.
   */
  brand: {
    ink: '#0A0A0C',
    bone: '#F6F4F0',
